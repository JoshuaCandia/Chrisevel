export const products = [
  {
    type: "ROSCADO",
    name: "Codo HH a 90º",
    category: "ROSCA",
    image: {
      public_id: "products/w5wklevdee8oligrtv5n",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756706/products/w5wklevdee8oligrtv5n.jpg",
    },
    sizes: ["1/2”", "3/4”", "1”", "1 1/4”", "1 1/2”", "2”"],
  },
  {
    type: "ROSCADO",
    name: "Codo MH a 90º",
    category: "ROSCA",
    image: {
      public_id: "products/l6ei8kus0zkzgvj4l4m1",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756708/products/l6ei8kus0zkzgvj4l4m1.jpg",
    },
    sizes: ["1/2”", "3/4”", "1”", "1 1/4”", "1 1/2”", "2”"],
  },
  {
    type: "ROSCADO",
    name: "Codo HH a 45º",
    category: "ROSCA",
    image: {
      public_id: "products/oupjboripihspf85i5j0",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697824065/products/oupjboripihspf85i5j0.jpg",
    },
    sizes: ["1/2”", "3/4”", "1”", "1 1/4”", "1 1/2”", "2”"],
  },
  {
    type: "ROSCADO",
    name: "Tee",
    category: "ROSCA",
    image: {
      public_id: "products/jiaqm0s0n5mpfiign1on",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756707/products/jiaqm0s0n5mpfiign1on.jpg",
    },
    sizes: ["1/2”", "3/4”", "1”", "1 1/4”", "1 1/2”", "2”"],
  },
  {
    type: "ROSCADO",
    name: "Unión doble",
    category: "ROSCA",
    image: {
      public_id: "products/pgvrzqskhufm68ibz77n",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756708/products/pgvrzqskhufm68ibz77n.jpg",
    },
    sizes: ["1/2”", "3/4”", "1”", "1 1/4”", "1 1/2”", "2”"],
  },
  {
    type: "ROSCADO",
    name: "Cupla HH",
    category: "ROSCA",
    image: {
      public_id: "products/l3ijafqsq6i34a8pztwo",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756708/products/l3ijafqsq6i34a8pztwo.jpg",
    },
    sizes: ["1/2”", "3/4”", "1”", "1 1/4”", "1 1/2”", "2”"],
  },
  {
    type: "ROSCADO",
    name: "Cupla MH",
    category: "ROSCA",
    image: {
      public_id: "products/okpitn78g1qk2uyosg80",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756708/products/okpitn78g1qk2uyosg80.jpg",
    },
    sizes: ["1/2”", "3/4”"],
  },
  {
    type: "ROSCADO",
    name: "Curva HH",
    category: "ROSCA",
    image: {
      public_id: "products/xypitfex7xgmew867byu",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756709/products/xypitfex7xgmew867byu.jpg",
    },
    sizes: ["1/2”", "3/4”", "1”", "1 1/4”", "1 1/2”", "2”"],
  },
  {
    type: "ROSCADO",
    name: "Curva MH",
    category: "ROSCA",
    image: {
      public_id: "products/vgqwxzmn3hzaa9uq5z07",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697795901/products/vgqwxzmn3hzaa9uq5z07.jpg",
    },
    sizes: ["1/2”", "3/4”", "1”", "1 1/4”", "1 1/2”", "2”"],
  },
  {
    type: "ROSCADO",
    name: "Rosca con tuerca",
    category: "ROSCA",
    image: {
      public_id: "products/b3opwwarfk71trypeonl",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756707/products/b3opwwarfk71trypeonl.jpg",
    },
    sizes: ["1/2”", "3/4”", "1”", "1 1/4”", "1 1/2”", "2”"],
  },
  {
    type: "ROSCADO",
    name: "Tapón",
    category: "ROSCA",
    image: {
      public_id: "products/hghuefqmvsict7vt4eha",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756707/products/hghuefqmvsict7vt4eha.jpg",
    },
    sizes: ["1/2”", "3/4”", "1”", "1 1/4”", "1 1/2”", "2”"],
  },
  {
    type: "ROSCADO",
    name: "Tapa",
    category: "ROSCA",
    image: {
      public_id: "products/yypmfyvufoguwas6isqv",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697797249/products/yypmfyvufoguwas6isqv.jpg",
    },
    sizes: ["1/2”", "3/4”", "1”", "1 1/4”", "1 1/2”", "2”"],
  },
  {
    type: "ROSCADO",
    name: "Curva Sobre Paso",
    category: "ROSCA",
    image: {
      public_id: "products/khshguwzpke5vpavrvhc",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756709/products/khshguwzpke5vpavrvhc.jpg",
    },
    sizes: ["1/2”", "3/4"],
  },
  {
    type: "ROSCADO",
    name: "Adaptador tanque",
    category: "ROSCA",
    image: {
      public_id: "products/kthudaopnlxib03nwu00",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756705/products/kthudaopnlxib03nwu00.jpg",
    },
    sizes: ["1/2”", "3/4”", "1”", "1 1/4”", "1 1/2”", "2”"],
  },
  {
    type: "ROSCADO",
    name: "Brida",
    category: "ROSCA",
    image: {
      public_id: "products/qpqxyk1tgznolbklsoj4",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756706/products/qpqxyk1tgznolbklsoj4.jpg",
    },
    sizes: ["1/2”", "3/4”", "1”", "1 1/4”", "1 1/2”", "2”"],
  },
  {
    type: "ROSCADO",
    name: "Junta PVC",
    category: "ROSCA",
    image: {
      public_id: "products/c7l2ar8csvxxeuifsc1s",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756707/products/c7l2ar8csvxxeuifsc1s.jpg",
    },
    sizes: ["1/2”", "3/4”", "1”", "1 1/4”", "1 1/2”", "2”"],
  },
  {
    type: "ROSCADO",
    name: "Acople Standard Largo",
    category: "ROSCA",
    image: {
      public_id: "products/tobug0ziy5iasqcnbdfe",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756705/products/tobug0ziy5iasqcnbdfe.jpg",
    },
    sizes: ["1/2”", "3/4”", "1”"],
  },
  {
    type: "ROSCADO",
    name: "Acople Standard Gris",
    category: "ROSCA",
    image: {
      public_id: "products/oojuiwi2ttpu9fxc2ipn",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756704/products/oojuiwi2ttpu9fxc2ipn.jpg",
    },
    sizes: ["1/2”", "3/4”", "1”"],
  },
  {
    type: "ROSCADO",
    name: "Acople profesional",
    category: "ROSCA",
    image: {
      public_id: "products/lb1nl70fsnslzqzb2t7e",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756705/products/lb1nl70fsnslzqzb2t7e.jpg",
    },
    sizes: ["1/2”", "3/4”"],
  },
  {
    type: "ROSCADO",
    name: "Cruz",
    category: "ROSCA",
    image: {
      public_id: "products/halnptl24igfygq1nzh3",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756708/products/halnptl24igfygq1nzh3.jpg",
    },
    sizes: ["1/2”", "3/4”", "1”"],
  },
  {
    type: "ROSCADO",
    name: "Codo HH 90º Red.",
    category: "ROSCA",
    image: {
      public_id: "products/e4jwdap4xojietqdiyjh",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697798893/products/e4jwdap4xojietqdiyjh.jpg",
    },
    sizes: ["3/4” x 1/2”", "1” x 1/2”", "1” x 3/4”"],
  },
  {
    type: "ROSCADO",
    name: "Cupla HH Red.",
    category: "ROSCA",
    image: {
      public_id: "products/jvrrbfftumzwsfu96yhb",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756709/products/jvrrbfftumzwsfu96yhb.jpg",
    },
    sizes: ["1/2” x 3/8”", "3/4” x 1/2”", "1” x 1/2”", "1” x 3/4”"],
  },
  {
    type: "ROSCADO",
    name: "Tee red.",
    category: "ROSCA",
    image: {
      public_id: "products/wjb10oexd0phy3kmi3z9",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756707/products/wjb10oexd0phy3kmi3z9.jpg",
    },
    sizes: ["3/4” x 1/2”", "1” x 1/2”", "1” x 3/4”"],
  },
  {
    type: "ROSCADO",
    name: "Rosca con tuerca red.",
    category: "ROSCA",
    image: {
      public_id: "products/djomsk3a7lxhgvmmzywd",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756708/products/djomsk3a7lxhgvmmzywd.jpg",
    },
    sizes: ["3/4” x 1/2”", "1 x 1/2”", "1 x 3/4"],
  },
  {
    type: "ROSCADO",
    name: "Buje",
    category: "ROSCA",
    image: {
      public_id: "products/yjnaaea3kqqcth5odoe3",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756706/products/yjnaaea3kqqcth5odoe3.jpg",
    },
    sizes: [
      "1/2” x 3/8”",

      "3/4” x 1/2”",

      "1” x 1/2”",

      "1” x 3/4”",

      "1 1/4” x 1/2”",

      "1 1/4” x 3/4”",

      "1 1/4” x 1”",

      "1 1/2” x 1/2”",

      "1 1/2” x 3/4”",

      "1 1/2” x 1”",

      "1 1/2” x 1 1/4”",

      "2” x 1/2”",

      "2” x 3/4”",

      "2” x 1”",

      "2” x 1 1/4”",

      "2” x 1 1/2”",
    ],
  },
  {
    type: "ROSCADO",
    name: "Niple x 6cm",
    category: "ROSCA",
    image: {
      public_id: "products/vkw23fba0dyacxw7ww0h",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756707/products/vkw23fba0dyacxw7ww0h.jpg",
    },
    sizes: [
      "1/2 X 6 CM",
      "1/2 X 8 CM",
      "1/2 X 10 CM",
      "1/2 X 12 CM",
      "1/2 X 15 CM",
      "1/2 X 20 CM",
      "1 X 6 CM",
      "1 X 8 CM",
      "1 X 10 CM",
      "1 X 12 CM",
      "1 X 15 CM",
      "1 X 20 CM",
      "1 1/4 X 10 CM",
      "1 1/4 X 12 CM",
      "1 1/4 X 15 CM",
      "1 1/4 X 20 CM",
      "1 1/2 X 10 CM",
      "1 1/2 X 12 CM",
      "1 1/2 X 15 CM",
      "1 1/2 X 20 CM",
      "2 X 10 CM",
      "2 X 12 CM",
      "2 X 15 CM",
      "2 X 20 CM",
      "1/2 X 25 CM",
      "3/4” X 25 CM",
      "1 X 25 CM",
    ],
  },

  {
    type: "ROSCADO",
    name: "Flexible gris",
    category: "ROSCA",
    image: {
      public_id: "products/fvjeetxoh6k2zcbwdgii",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756702/products/fvjeetxoh6k2zcbwdgii.jpg",
    },
    sizes: ["1/2 X 30 CM", "1/2 X 40 CM", "1/2 X 50 CM"],
  },
  {
    type: "ESPIGA",
    name: "Enchufe doble",
    category: "RIEGO",
    image: {
      public_id: "products/s0zjkgs3a300u9reou97",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756709/products/s0zjkgs3a300u9reou97.jpg",
    },
    sizes: ["1/2”", "3/4”", "1”", "1 1/4”", "1 1/2”", "2”", "2 1/2”"],
  },
  {
    type: "ESPIGA",
    name: "Enchufe rosca macho",
    category: "RIEGO",
    image: {
      public_id: "products/ryqbjozk6l0kkcxzwxdh",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756703/products/ryqbjozk6l0kkcxzwxdh.jpg",
    },
    sizes: ["1/2”", "3/4”", "1”", "1 1/4”", "1 1/2”", "2”", "2 1/2”"],
  },
  {
    type: "ESPIGA",
    name: "Enchufe rosca hembra",
    category: "RIEGO",
    image: {
      public_id: "products/xmsshj8g8pjazrnxcirb",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756709/products/xmsshj8g8pjazrnxcirb.jpg",
    },
    sizes: ["1/2”", "3/4”", "1”", "1 1/4”", "1 1/2”", "2”"],
  },
  {
    type: "ESPIGA",
    name: "Codo enchufe doble",
    category: "RIEGO",
    image: {
      public_id: "products/fyqgajz2emphmyib8zmh",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756706/products/fyqgajz2emphmyib8zmh.jpg",
    },
    sizes: ["1/2”", "3/4”", "1”", "1 1/4”", "1 1/2”", "2”"],
  },

  {
    type: "ESPIGA",
    name: "Codo enchufe rosca hembra",
    category: "RIEGO",
    image: {
      public_id: "products/sn2w19cc1tak9jywsm6p",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756708/products/sn2w19cc1tak9jywsm6p.jpg",
    },
    sizes: ["1/2”", "3/4”", "1”", "1 1/4”", "1 1/2”", "2”"],
  },
  {
    type: "ESPIGA",
    name: "Codo enchufe rosca macho",
    category: "RIEGO",
    image: {
      public_id: "products/itlg93dpwp1hnax1pkmf",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756705/products/itlg93dpwp1hnax1pkmf.jpg",
    },
    sizes: ["1/2”"],
  },
  {
    type: "ESPIGA",
    name: "Enchufe doble red",
    category: "RIEGO",
    image: {
      public_id: "products/vx1erpcn2ruvm1ggjnj5",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756709/products/vx1erpcn2ruvm1ggjnj5.jpg",
    },
    sizes: ["3/4” X 1/2”", "1” X 3/4”", "1” X 1/2”"],
  },
  {
    type: "ESPIGA",
    name: "Tee triple",
    category: "RIEGO",
    image: {
      public_id: "products/hbb8xjxd1wzfbqcpfgnv",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756705/products/hbb8xjxd1wzfbqcpfgnv.jpg",
    },
    sizes: ["1/2”", "3/4”", "1”", "1 1/4”", "1 1/2”", "2”"],
  },
  {
    type: "ESPIGA",
    name: "Tee rosca hembra",
    category: "RIEGO",
    image: {
      public_id: "products/cxv5idh0w1dh47egd2ln",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756705/products/cxv5idh0w1dh47egd2ln.jpg",
    },
    sizes: ["1/2”", "3/4”", "1”", "1 1/4”", "1 1/2”", "2”"],
  },
  {
    type: "ESPIGA",
    name: "Enchufe rosca macho red",
    category: "RIEGO",
    image: {
      public_id: "products/ryqbjozk6l0kkcxzwxdh",
      secure_url:
        "https://http2.mlstatic.com/D_NQ_NP_917992-MLA47886449535_102021-O.webp",
    },
    sizes: ["3/4” X 1/2”", "1” X  3/4”"],
  },
  {
    type: "KRONA",
    name: "Valvula a esfera, rosca HH",
    category: "VALVULA",
    image: {
      public_id: "products/rgvszl9tpfu7f47ulhzo",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756704/products/rgvszl9tpfu7f47ulhzo.jpg",
    },
    sizes: [
      "1/2”",
      "3/4”",
      "1”",
      "1 1/4”",
      "1 1/2”",
      "2”",
      "2 1/2”",
      "3”",
      "4”",
    ],
  },
  {
    type: "KRONA",
    name: "Valvula de retención c/canasto",
    category: "VALVULA",
    image: {
      public_id: "products/dy2cdxxkurlorxroyft6",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756703/products/dy2cdxxkurlorxroyft6.jpg",
    },
    sizes: ["3/4”", "1”", "1 1/2”", "2”"],
  },
  {
    type: "KRONA",
    name: "Val. Antirretorno con adap.",
    category: "VALVULA",
    image: {
      public_id: "products/xks1rc8txdg2xfdwp5pv",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697837397/products/xks1rc8txdg2xfdwp5pv.jpg",
    },
    sizes: ["110 MM"],
  },
  {
    type: "KRONA",
    name: "Canilla plástica Jardín",
    category: "VALVULA",
    image: {
      public_id: "products/ohs9mvbvnh0w8sisc5m6",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697820993/products/ohs9mvbvnh0w8sisc5m6.jpg",
    },
    sizes: ["1/2” 3/4” PVC"],
  },
  {
    type: "KRONA",
    name: "Válvulas a esfera con media unión",
    category: "VALVULA",
    image: {
      public_id: "products/dktxjwggvbwjsytkjemw",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756703/products/dktxjwggvbwjsytkjemw.jpg",
    },
    sizes: ["1/2”", "3/4”", "1”", "1 1/4”", "1 1/2”", "2”", "40 MM", "50 MM"],
  },
  {
    type: "RIEGO",
    name: "Adaptador soldable rosca macho",
    category: "PVC",
    image: {
      public_id: "products/wu6k8pchsz1qll47vbhv",
      secure_url:
        "https://www.tiendalincoln.com.py/static/products/DEFAULT/G07002/SMALL/G07002%20ADAPTA.ROS%20SOLD.Krona%20332%2032x1.jpg",
    },
    sizes: [
      "20 MM x 1/2”",
      "25 MM x 3/4”",
      "32 MM x 1”",
      "40 MM x 1 1/4”",
      "50 MM x 1 1/2”",
    ],
  },
  {
    type: "KRONA",
    name: "Buje Corto soldable",
    category: "PVC",
    image: {
      public_id: "products/bbfqg58jgjvcqvhek2fo",
      secure_url:
        "https://http2.mlstatic.com/D_NQ_NP_689046-MLU74088849550_012024-O.webphttps://www.tiendalincoln.com.py/static/products/DEFAULT/G07015/SMALL/G07015%20BUJE%20RED.COR.SO.Krona%20364%2060x50m.jpg",
    },
    sizes: ["25 x 20 MM", "32 x 25 MM", "40 x 32 MM", "50 x 40 MM"],
  },
  {
    type: "KRONA",
    name: "Curva soldable a 90°",
    category: "PVC",
    image: {
      public_id: "products/wkicciarpa4jyo8hla7v",
      secure_url:
        "https://dcdn.mitiendanube.com/stores/864/808/products/curva-90-a-50-imagen-tienda-nube-092-90f934a27c210081f116328628322272-1024-1024.webp",
    },
    sizes: ["20 MM", "25 MM", "32 MM", "40 MM", "50 MM"],
  },
  {
    type: "KRONA",
    name: "Codo soldable a 45°",
    category: "PVC",
    image: {
      public_id: "products/xd3ufqkifl9ly4zdvasb",
      secure_url:
        "https://www.tiendalincoln.com.py/static/products/DEFAULT/G07054/SMALL/G07054%20CODO%20%2045%C2%AA%20SOLDA.Krona%20419%2050mm.jpg",
    },
    sizes: ["20 MM", "25 MM", "32 MM", "40 MM", "50 MM"],
  },
  {
    type: "KRONA",
    name: "Codo soldable a 90°",
    category: "PVC",
    image: {
      public_id: "products/xulgxrjh08f5vbdr4nvv",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697823133/products/xulgxrjh08f5vbdr4nvv.jpg",
    },
    sizes: ["20 MM", "25 MM", "32 MM", "40 MM", "50 MM"],
  },
  {
    type: "KRONA",
    name: "Cupla soldable",
    category: "PVC",
    image: {
      public_id: "products/topgqeidfvbj2w7hbwj7",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697838273/products/topgqeidfvbj2w7hbwj7.jpg",
    },
    sizes: ["20 MM", "25 MM", "32 MM", "40 MM", "50 MM"],
  },
  {
    type: "KRONA",
    name: "Tee soldable",
    category: "PVC",
    image: {
      public_id: "products/jtpm4ucberabr9o80wku",
      secure_url:
        "https://www.tiendalincoln.com.py/static/products/DEFAULT/G07091/SMALL/G07091%20TEE%20SOLDAR%20%20Krona%20455%2025mm.jpg",
    },
    sizes: ["20 MM", "25 MM", "32 MM", "40 MM", "50 MM"],
  },
  {
    type: "KRONA",
    name: "Unión doble soldable",
    category: "PVC",
    image: {
      public_id: "products/q80j1pphi4zjj8tbzlut",
      secure_url:
        "https://www.tiendalincoln.com.py/static/products/DEFAULT/G07115/SMALL/G07115%20UNION%20DOBLE%20SOL.Krona%20479%2060mm.jpg",
    },
    sizes: ["20 MM", "25 MM", "32 MM", "40 MM", "50 MM"],
  },
  {
    type: "KRONA",
    name: "Codo a 90° rosca hembra soldable",
    category: "PVC",
    image: {
      public_id: "products/doxwzipphszkl2iq79ul",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756703/products/doxwzipphszkl2iq79ul.jpg",
    },
    sizes: ["20 MM x 1/2”", "25 MM x 3/4”"],
  },
  {
    type: "KRONA",
    name: "Adaptador soldable rosca hembra",
    category: "PVC",
    image: {
      public_id: "products/fijvppuk3wl1uc2inzru",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756703/products/fijvppuk3wl1uc2inzru.jpg",
    },
    sizes: [
      "20 MM x 1/2”",

      "25 MM x 3/4”",

      "32 MM x 1”",

      "40 MM x 1 1/4”",

      "50 MM x 1 1/2”",
    ],
  },
  {
    type: "KRONA",
    name: "Montura",
    category: "PVC",
    image: {
      public_id: "products/aqmk8yvlamscff0nnwdn",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697822874/products/aqmk8yvlamscff0nnwdn.jpg",
    },
    sizes: ["50 x 1/2”", "50 x 3/4”"],
  },
  {
    type: "FUSION",
    name: "Codo a 90°",
    category: "FUSION",
    image: {
      public_id: "products/iszkrze3jegj12wde6ax",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756704/products/iszkrze3jegj12wde6ax.jpg",
    },
    sizes: ["20 MM", "25 MM", "32 MM"],
  },
  {
    type: "FUSION",
    name: "Tee",
    category: "FUSION",
    image: {
      public_id: "products/jiaqm0s0n5mpfiign1on",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756704/products/jtpm4ucberabr9o80wku.jpg",
    },
    sizes: ["20 MM", "25 MM", "32 MM"],
  },
  {
    type: "FUSION",
    name: "Cupla",
    category: "FUSION",
    image: {
      public_id: "products/bbfqg58jgjvcqvhek2fo",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756704/products/bbfqg58jgjvcqvhek2fo.jpg",
    },
    sizes: ["20 MM", "25 MM", "32 MM"],
  },
  {
    type: "FUSION",
    name: "Tapa",
    category: "FUSION",
    image: {
      public_id: "products/jiaqm0s0n5mpfiign1on",
      secure_url:
        "https://acdn.mitiendanube.com/stores/001/230/396/products/1-tapa-e05b0b953b4e07ced517131430510330-640-0.jpg",
    },
    sizes: ["20 MM", "25 MM", "32 MM"],
  },
  {
    type: "FUSION",
    name: "Unión doble",
    category: "FUSION",
    image: {
      public_id: "products/q80j1pphi4zjj8tbzlut",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756704/products/q80j1pphi4zjj8tbzlut.jpg",
    },
    sizes: ["20 MM", "25 MM", "32 MM", "40 MM", "50 MM", "63 MM"],
  },
  {
    type: "FUSION",
    name: "Unión doble con rosca plástica",
    category: "FUSION",
    image: {
      public_id: "products/q80j1pphi4zjj8tbzlut",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1697756704/products/q80j1pphi4zjj8tbzlut.jpg",
    },
    sizes: [
      "20 MM x 1/2”",
      "25 MM x 3/4”",
      "32 MM x 1”",
      "40 MM x 11/4”",
      "50 MM x 11/2”",
      "63 MM x 2”",
    ],
  },
  {
    type: "FUSION",
    name: "Buje reducción",
    category: "FUSION",
    image: {
      public_id: "products/q80j1pphi4zjj8tbzlut",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1723169120/latest-products/IMAGENES_CORTADAS/ln7u6lo0fppxh2coylp5.png",
    },
    sizes: ["25 MM x 20 MM", "32 MM X 20 MM", "32 MM x 25 MM"],
  },
  {
    type: "FUSION",
    name: "Curva a 90°",
    category: "FUSION",
    image: {
      public_id: "products/iszkrze3jegj12wde6ax",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1723169118/latest-products/IMAGENES_CORTADAS/aygadnssxfw3cvxpogbg.png",
    },
    sizes: ["20 MM", "25 MM", "32 MM"],
  },
  {
    type: "FUSION",
    name: "Adaptador tanque",
    category: "FUSION",
    image: {
      public_id: "products/iszkrze3jegj12wde6ax",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1723169118/latest-products/IMAGENES_CORTADAS/ajlbf6jnziasbckk7oex.png",
    },
    sizes: ["20 MM", "25 MM", "32 MM", "40MM", "50MM", "63MM"],
  },

  {
    type: "FUSION",
    name: "Cupla rosca-fusión",
    category: "FUSION",
    image: {
      public_id: "products/q80j1pphi4zjj8tbzlut",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1723169118/latest-products/IMAGENES_CORTADAS/ke5uvg6cq9nbdmoonml2.png",
    },
    sizes: ["20 x 1/2”", "25 3/4”"],
  },
  {
    type: "FUSION",
    name: "Codo rosca-fusión",
    category: "FUSION",
    image: {
      public_id: "products/q80j1pphi4zjj8tbzlut",
      secure_url:
        "https://res.cloudinary.com/djbeg0zrq/image/upload/v1723169118/latest-products/IMAGENES_CORTADAS/qlkm5f4bka3tymy2jvr6.png",
    },
    sizes: ["20 x 1/2”"],
  },
  {
    type: "FUSION",
    name: "Codo inserción hembra",
    category: "FUSION",
    image: {
      public_id: "products/q80j1pphi4zjj8tbzlut",
      secure_url:
        "https://giliycia.com.ar/media/catalog/product/cache/99c3ac325871812be19f74bdf645f821/0/4/04713.jpg",
    },
    sizes: ["20 x 1/2”", "25 3/4”", "32 x 1”"],
  },
  {
    type: "FUSION",
    name: "Codo inserción macho",
    category: "FUSION",
    image: {
      public_id: "products/q80j1pphi4zjj8tbzlut",
      secure_url:
        "https://casamyers.com.mx/img/ItemImages/28301440/28301440.jpg",
    },
    sizes: ["20 x 1/2”", "25 3/4”", "32 x 1”"],
  },
  {
    type: "FUSION",
    name: "Cupla inserción hembra",
    category: "FUSION",
    image: {
      public_id: "products/q80j1pphi4zjj8tbzlut",
      secure_url:
        "https://acdn.mitiendanube.com/stores/002/897/767/products/cupla-con-insert-381-0768d5aa5043411c5f16819300275545-1024-1024.webp",
    },
    sizes: ["20 x 1/2”", "25 3/4”", "32 x 1”"],
  },
  {
    type: "FUSION",
    name: "Cupla inserción macho",
    category: "FUSION",
    image: {
      public_id: "products/q80j1pphi4zjj8tbzlut",
      secure_url:
        "https://http2.mlstatic.com/D_NQ_NP_668407-MLA45363061630_032021-O.webp",
    },
    sizes: ["20 x 1/2”", "25 3/4”", "32 x 1”"],
  },
  {
    type: "FUSION",
    name: "Tee inserción macho",
    category: "FUSION",
    image: {
      public_id: "products/q80j1pphi4zjj8tbzlut",
      secure_url:
        "https://casamyers.com.mx/img/ItemImages/28301650/28301650.jpg",
    },
    sizes: ["20 x 1/2”", "25 3/4”", "32 x 1”"],
  },

  {
    type: "FUSION",
    name: "Llave de paso (BCE)",
    category: "FUSION",
    image: {
      public_id: "products/q80j1pphi4zjj8tbzlut",
      secure_url:
        "https://http2.mlstatic.com/D_NQ_NP_868113-MLA54304543284_032023-O.webp",
    },
    sizes: ["20 MM", "25 MM"],
  },
];
